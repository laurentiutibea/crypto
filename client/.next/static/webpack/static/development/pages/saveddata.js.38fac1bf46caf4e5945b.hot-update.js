webpackHotUpdate("static\\development\\pages\\saveddata.js",{

/***/ "./pages/saveddata.js":
/*!****************************!*\
  !*** ./pages/saveddata.js ***!
  \****************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return SavedData; });
/* harmony import */ var _babel_runtime_corejs2_core_js_object_define_property__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @babel/runtime-corejs2/core-js/object/define-property */ "./node_modules/@babel/runtime-corejs2/core-js/object/define-property.js");
/* harmony import */ var _babel_runtime_corejs2_core_js_object_define_property__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_core_js_object_define_property__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _babel_runtime_corejs2_core_js_object_define_properties__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @babel/runtime-corejs2/core-js/object/define-properties */ "./node_modules/@babel/runtime-corejs2/core-js/object/define-properties.js");
/* harmony import */ var _babel_runtime_corejs2_core_js_object_define_properties__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_core_js_object_define_properties__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _babel_runtime_corejs2_core_js_object_get_own_property_descriptors__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @babel/runtime-corejs2/core-js/object/get-own-property-descriptors */ "./node_modules/@babel/runtime-corejs2/core-js/object/get-own-property-descriptors.js");
/* harmony import */ var _babel_runtime_corejs2_core_js_object_get_own_property_descriptors__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_core_js_object_get_own_property_descriptors__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _babel_runtime_corejs2_core_js_object_get_own_property_descriptor__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @babel/runtime-corejs2/core-js/object/get-own-property-descriptor */ "./node_modules/@babel/runtime-corejs2/core-js/object/get-own-property-descriptor.js");
/* harmony import */ var _babel_runtime_corejs2_core_js_object_get_own_property_descriptor__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_core_js_object_get_own_property_descriptor__WEBPACK_IMPORTED_MODULE_3__);
/* harmony import */ var _babel_runtime_corejs2_core_js_object_get_own_property_symbols__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @babel/runtime-corejs2/core-js/object/get-own-property-symbols */ "./node_modules/@babel/runtime-corejs2/core-js/object/get-own-property-symbols.js");
/* harmony import */ var _babel_runtime_corejs2_core_js_object_get_own_property_symbols__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_core_js_object_get_own_property_symbols__WEBPACK_IMPORTED_MODULE_4__);
/* harmony import */ var _babel_runtime_corejs2_core_js_object_keys__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! @babel/runtime-corejs2/core-js/object/keys */ "./node_modules/@babel/runtime-corejs2/core-js/object/keys.js");
/* harmony import */ var _babel_runtime_corejs2_core_js_object_keys__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_core_js_object_keys__WEBPACK_IMPORTED_MODULE_5__);
/* harmony import */ var _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! @babel/runtime-corejs2/regenerator */ "./node_modules/@babel/runtime-corejs2/regenerator/index.js");
/* harmony import */ var _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6__);
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/toConsumableArray */ "./node_modules/@babel/runtime-corejs2/helpers/esm/toConsumableArray.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/asyncToGenerator */ "./node_modules/@babel/runtime-corejs2/helpers/esm/asyncToGenerator.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_classCallCheck__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/classCallCheck */ "./node_modules/@babel/runtime-corejs2/helpers/esm/classCallCheck.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_createClass__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/createClass */ "./node_modules/@babel/runtime-corejs2/helpers/esm/createClass.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_possibleConstructorReturn__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/possibleConstructorReturn */ "./node_modules/@babel/runtime-corejs2/helpers/esm/possibleConstructorReturn.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_getPrototypeOf__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/getPrototypeOf */ "./node_modules/@babel/runtime-corejs2/helpers/esm/getPrototypeOf.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/assertThisInitialized */ "./node_modules/@babel/runtime-corejs2/helpers/esm/assertThisInitialized.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_inherits__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/inherits */ "./node_modules/@babel/runtime-corejs2/helpers/esm/inherits.js");
/* harmony import */ var _babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(/*! @babel/runtime-corejs2/helpers/esm/defineProperty */ "./node_modules/@babel/runtime-corejs2/helpers/esm/defineProperty.js");
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(/*! react */ "./node_modules/react/index.js");
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_16___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_16__);
/* harmony import */ var _components_layout__WEBPACK_IMPORTED_MODULE_17__ = __webpack_require__(/*! ../components/layout */ "./components/layout.js");
/* harmony import */ var highcharts__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(/*! highcharts */ "./node_modules/highcharts/highcharts.js");
/* harmony import */ var highcharts__WEBPACK_IMPORTED_MODULE_18___default = /*#__PURE__*/__webpack_require__.n(highcharts__WEBPACK_IMPORTED_MODULE_18__);
/* harmony import */ var highcharts_react_official__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(/*! highcharts-react-official */ "./node_modules/highcharts-react-official/dist/highcharts-react.min.js");
/* harmony import */ var highcharts_react_official__WEBPACK_IMPORTED_MODULE_19___default = /*#__PURE__*/__webpack_require__.n(highcharts_react_official__WEBPACK_IMPORTED_MODULE_19__);
/* harmony import */ var _services_authService__WEBPACK_IMPORTED_MODULE_20__ = __webpack_require__(/*! ../services/authService */ "./services/authService.js");
/* harmony import */ var _services_cryptoService__WEBPACK_IMPORTED_MODULE_21__ = __webpack_require__(/*! ../services/cryptoService */ "./services/cryptoService.js");
/* harmony import */ var _src_91_gif__WEBPACK_IMPORTED_MODULE_22__ = __webpack_require__(/*! ../src/91.gif */ "./src/91.gif");
/* harmony import */ var _src_91_gif__WEBPACK_IMPORTED_MODULE_22___default = /*#__PURE__*/__webpack_require__.n(_src_91_gif__WEBPACK_IMPORTED_MODULE_22__);
/* harmony import */ var _src_crypto_text_png__WEBPACK_IMPORTED_MODULE_23__ = __webpack_require__(/*! ../src/crypto-text.png */ "./src/crypto-text.png");
/* harmony import */ var _src_crypto_text_png__WEBPACK_IMPORTED_MODULE_23___default = /*#__PURE__*/__webpack_require__.n(_src_crypto_text_png__WEBPACK_IMPORTED_MODULE_23__);
















var _jsxFileName = "D:\\WEB\\crypto\\client\\pages\\saveddata.js";
var __jsx = react__WEBPACK_IMPORTED_MODULE_16___default.a.createElement;

function ownKeys(object, enumerableOnly) { var keys = _babel_runtime_corejs2_core_js_object_keys__WEBPACK_IMPORTED_MODULE_5___default()(object); if (_babel_runtime_corejs2_core_js_object_get_own_property_symbols__WEBPACK_IMPORTED_MODULE_4___default.a) { var symbols = _babel_runtime_corejs2_core_js_object_get_own_property_symbols__WEBPACK_IMPORTED_MODULE_4___default()(object); if (enumerableOnly) symbols = symbols.filter(function (sym) { return _babel_runtime_corejs2_core_js_object_get_own_property_descriptor__WEBPACK_IMPORTED_MODULE_3___default()(object, sym).enumerable; }); keys.push.apply(keys, symbols); } return keys; }

function _objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i] != null ? arguments[i] : {}; if (i % 2) { ownKeys(Object(source), true).forEach(function (key) { Object(_babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__["default"])(target, key, source[key]); }); } else if (_babel_runtime_corejs2_core_js_object_get_own_property_descriptors__WEBPACK_IMPORTED_MODULE_2___default.a) { _babel_runtime_corejs2_core_js_object_define_properties__WEBPACK_IMPORTED_MODULE_1___default()(target, _babel_runtime_corejs2_core_js_object_get_own_property_descriptors__WEBPACK_IMPORTED_MODULE_2___default()(source)); } else { ownKeys(Object(source)).forEach(function (key) { _babel_runtime_corejs2_core_js_object_define_property__WEBPACK_IMPORTED_MODULE_0___default()(target, key, _babel_runtime_corejs2_core_js_object_get_own_property_descriptor__WEBPACK_IMPORTED_MODULE_3___default()(source, key)); }); } } return target; }









var chartOptions = {
  chart: {
    zoomType: 'x'
  },
  xAxis: {
    type: 'datetime',
    dateTimeLabelFormats: {
      day: '%d-%m-%Y - %h:%m'
    },
    title: {
      text: 'Date'
    }
  },
  yAxis: {
    title: {
      text: 'Exchange rate'
    }
  },
  legend: {
    enabled: false
  },
  plotOptions: {
    area: {
      fillColor: {
        linearGradient: {
          x1: 0,
          y1: 0,
          x2: 0,
          y2: 1
        },
        stops: [[0, "#7cb5ec"], [0.5, "#7cb5ec80"], [0.9, "#7cb5ff33"]]
      },
      marker: {
        radius: 2
      },
      lineWidth: 1,
      states: {
        hover: {
          lineWidth: 1
        }
      },
      threshold: null
    }
  }
};

var SavedData =
/*#__PURE__*/
function (_Component) {
  Object(_babel_runtime_corejs2_helpers_esm_inherits__WEBPACK_IMPORTED_MODULE_14__["default"])(SavedData, _Component);

  function SavedData() {
    var _getPrototypeOf2;

    var _this;

    Object(_babel_runtime_corejs2_helpers_esm_classCallCheck__WEBPACK_IMPORTED_MODULE_9__["default"])(this, SavedData);

    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    _this = Object(_babel_runtime_corejs2_helpers_esm_possibleConstructorReturn__WEBPACK_IMPORTED_MODULE_11__["default"])(this, (_getPrototypeOf2 = Object(_babel_runtime_corejs2_helpers_esm_getPrototypeOf__WEBPACK_IMPORTED_MODULE_12__["default"])(SavedData)).call.apply(_getPrototypeOf2, [this].concat(args)));

    Object(_babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__["default"])(Object(_babel_runtime_corejs2_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_13__["default"])(_this), "state", {
      user: {},
      jwt: "",
      data: {
        graphs: []
      },
      loaded: false,
      graphs: [],
      loadArr: [],
      compare: false
    });

    Object(_babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__["default"])(Object(_babel_runtime_corejs2_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_13__["default"])(_this), "refreshData",
    /*#__PURE__*/
    Object(_babel_runtime_corejs2_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_8__["default"])(
    /*#__PURE__*/
    _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.mark(function _callee() {
      var graphs, loadArr, _ref2, data, _i, bkOptions;

      return _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              graphs = [];
              loadArr = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.loadArr);
              if (_this.state.graphs.length === 0) graphs = [];else _this.setState({
                graphs: []
              });
              _context.next = 5;
              return _services_cryptoService__WEBPACK_IMPORTED_MODULE_21__["default"].getSavedData(_this.state.user._id);

            case 5:
              _ref2 = _context.sent;
              data = _ref2.data;

              _this.setState({
                loaded: false
              });

              for (_i = 0; _i < data.graphs.length; _i++) {
                bkOptions = _objectSpread({}, chartOptions);
                bkOptions.title = {
                  text: "".concat(data.graphs[_i].cryptocurrency.name, " to ").concat(data.graphs[_i].currency.name, " exchange rate over time")
                };
                bkOptions.series = [{
                  type: "line",
                  name: "".concat(data.graphs[_i].cryptocurrency.name, " to ").concat(data.graphs[_i].currency.name),
                  data: data.graphs[_i].series.data,
                  connectNulls: true
                }];
                bkOptions.xAxis.categories = data.graphs[_i].categories;
                graphs.push({
                  chartOptions: bkOptions,
                  cryptocurrency: {
                    name: data.graphs[_i].cryptocurrency.name,
                    value: data.graphs[_i].cryptocurrency.value
                  },
                  currency: {
                    name: data.graphs[_i].currency.name,
                    value: data.graphs[_i].currency.value
                  },
                  live: false,
                  image: data.graphs[_i].image,
                  graphId: data.graphs[_i].graphId,
                  date: data.graphs[_i].date
                });
                loadArr.push(false);
              }

              _this.setState({
                data: data,
                loaded: true,
                graphs: graphs,
                loadArr: loadArr
              });

            case 10:
            case "end":
              return _context.stop();
          }
        }
      }, _callee);
    })));

    Object(_babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__["default"])(Object(_babel_runtime_corejs2_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_13__["default"])(_this), "refreshSingle",
    /*#__PURE__*/
    function () {
      var _ref3 = Object(_babel_runtime_corejs2_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_8__["default"])(
      /*#__PURE__*/
      _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.mark(function _callee2(index) {
        var _ref4, data, loadArr, graphs, bkOptions;

        return _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                _context2.next = 2;
                return _services_cryptoService__WEBPACK_IMPORTED_MODULE_21__["default"].getSavedData(_this.state.user._id);

              case 2:
                _ref4 = _context2.sent;
                data = _ref4.data;
                loadArr = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.loadArr);
                loadArr[index] = true;

                _this.setState({
                  loadArr: loadArr
                });

                graphs = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.graphs);
                bkOptions = _objectSpread({}, chartOptions);
                bkOptions.title = {
                  text: "".concat(data.graphs[i].cryptocurrency.name, " to ").concat(data.graphs[i].currency.name, " exchange rate over time")
                };
                bkOptions.series = [{
                  type: "line",
                  name: "".concat(data.graphs[i].cryptocurrency.name, " to ").concat(data.graphs[i].currency.name),
                  data: data.graphs[i].series.data,
                  connectNulls: true
                }];
                bkOptions.xAxis.categories = data.graphs[i].categories;
                graphs.push({
                  chartOptions: bkOptions,
                  cryptocurrency: {
                    name: data.graphs[i].cryptocurrency.name,
                    value: data.graphs[i].cryptocurrency.value
                  },
                  currency: {
                    name: data.graphs[i].currency.name,
                    value: data.graphs[i].currency.value
                  },
                  live: false,
                  image: data.graphs[i].image,
                  graphId: data.graphs[i].graphId,
                  date: data.graphs[i].date
                });
                loadArr[index] = false;

                _this.setState({
                  data: data,
                  loaded: true,
                  graphs: graphs,
                  loadArr: loadArr
                });

              case 15:
              case "end":
                return _context2.stop();
            }
          }
        }, _callee2);
      }));

      return function (_x) {
        return _ref3.apply(this, arguments);
      };
    }());

    Object(_babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__["default"])(Object(_babel_runtime_corejs2_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_13__["default"])(_this), "compare",
    /*#__PURE__*/
    function () {
      var _ref5 = Object(_babel_runtime_corejs2_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_8__["default"])(
      /*#__PURE__*/
      _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.mark(function _callee3(e, index) {
        var worker, graphs, loadArr, _graphs, _loadArr;

        return _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.wrap(function _callee3$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                if (!e.target.checked) {
                  _context3.next = 11;
                  break;
                }

                worker = new Worker("static/service-worker.js");
                graphs = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.graphs);
                console.log(graphs);
                loadArr = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.loadArr);
                loadArr[index] = true;

                _this.setState({
                  loadArr: loadArr
                });

                worker.onmessage = function (e) {
                  graphs[index].chartOptions.series.push({
                    type: "line",
                    name: "NEW:" + _this.state.graphs[index].chartOptions.series[0].name,
                    data: e.data.prices,
                    _colorIndex: 1,
                    connectNulls: true,
                    pointStart: 1,
                    pointInterval: 2
                  });
                  var dates = [];

                  for (var _i2 = 0; _i2 < e.data.time.length; _i2++) {
                    dates.push(graphs[index].chartOptions.xAxis.categories[_i2]);
                    dates.push(e.data.time[_i2]);
                  }

                  graphs[index].chartOptions.xAxis.categories = [].concat(dates);
                  graphs[index].chartOptions.series[0].pointInterval = 2;
                  loadArr[index] = false;

                  _this.setState({
                    graphs: graphs,
                    loadArr: loadArr,
                    compare: true
                  });

                  console.log(graphs[index].chartOptions.xAxis.categories);
                };

                worker.postMessage("".concat(graphs[index].cryptocurrency.value, ",").concat(graphs[index].currency.value, ",").concat(_this.state.jwt));
                _context3.next = 21;
                break;

              case 11:
                _graphs = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.graphs);
                _loadArr = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.loadArr);
                _loadArr[index] = true;
                _context3.next = 16;
                return _this.setState({
                  loadArr: _loadArr
                });

              case 16:
                _graphs[index].chartOptions.title = {
                  text: "".concat(_graphs[index].cryptocurrency.name, " to ").concat(_graphs[index].currency.name, " exchange rate over time")
                };
                _graphs[index].chartOptions.series = [{
                  type: "line",
                  name: "".concat(_graphs[index].cryptocurrency.name, " to ").concat(_graphs[index].currency.name),
                  data: _graphs[index].chartOptions.series[0].data,
                  connectNulls: true
                }];
                if (_graphs[index].chartOptions.xAxis.categories.length > 9) _graphs[index].chartOptions.xAxis.categories = _graphs[index].chartOptions.xAxis.categories.filter(function (item, index) {
                  return index % 2 === 0;
                });
                _loadArr[index] = false;

                _this.setState({
                  graphs: _graphs,
                  loadArr: _loadArr,
                  compare: false
                });

              case 21:
              case "end":
                return _context3.stop();
            }
          }
        }, _callee3);
      }));

      return function (_x2, _x3) {
        return _ref5.apply(this, arguments);
      };
    }());

    Object(_babel_runtime_corejs2_helpers_esm_defineProperty__WEBPACK_IMPORTED_MODULE_15__["default"])(Object(_babel_runtime_corejs2_helpers_esm_assertThisInitialized__WEBPACK_IMPORTED_MODULE_13__["default"])(_this), "editGraph",
    /*#__PURE__*/
    function () {
      var _ref6 = Object(_babel_runtime_corejs2_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_8__["default"])(
      /*#__PURE__*/
      _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.mark(function _callee4(index, action) {
        var graphs, bkGraphs, bkCategories, graph;
        return _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.wrap(function _callee4$(_context4) {
          while (1) {
            switch (_context4.prev = _context4.next) {
              case 0:
                graphs = Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_7__["default"])(_this.state.graphs);
                if (action === "remove") graphs = graphs.filter(function (item, i) {
                  return i !== index;
                });
                bkGraphs = [];
                bkCategories = [];
                graphs.forEach(function (element, i) {
                  if (i === index) bkCategories = element.chartOptions.xAxis.categories.filter(function (item, index) {
                    return index % 2 !== 0;
                  });else if (element.chartOptions.xAxis.categories.length > 9) bkCategories = element.chartOptions.xAxis.categories.filter(function (item, index) {
                    return index % 2 === 0;
                  });else bkCategories = element.chartOptions.xAxis.categories;
                  bkGraphs.push({
                    categories: bkCategories,
                    series: action === "save" ? i === index ? element.chartOptions.series[1] : element.chartOptions.series[0] : element.chartOptions.series[0],
                    graphId: element.graphId,
                    cryptocurrency: {
                      value: element.cryptocurrency.value,
                      name: element.cryptocurrency.name
                    },
                    currency: {
                      value: element.currency.value,
                      name: element.currency.name
                    },
                    date: new Date(),
                    image: element.image
                  });
                });
                graph = {
                  userId: _this.state.user._id,
                  graphs: bkGraphs
                };
                console.log(bkGraphs);
                console.log(graph); //await crypto.replaceGraphs(graph);

                _this.setState({
                  graphs: graphs
                });

                _this.refreshSingle(index);

              case 10:
              case "end":
                return _context4.stop();
            }
          }
        }, _callee4);
      }));

      return function (_x4, _x5) {
        return _ref6.apply(this, arguments);
      };
    }());

    return _this;
  }

  Object(_babel_runtime_corejs2_helpers_esm_createClass__WEBPACK_IMPORTED_MODULE_10__["default"])(SavedData, [{
    key: "componentDidMount",
    value: function () {
      var _componentDidMount = Object(_babel_runtime_corejs2_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_8__["default"])(
      /*#__PURE__*/
      _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.mark(function _callee5() {
        var user, jwt;
        return _babel_runtime_corejs2_regenerator__WEBPACK_IMPORTED_MODULE_6___default.a.wrap(function _callee5$(_context5) {
          while (1) {
            switch (_context5.prev = _context5.next) {
              case 0:
                _context5.next = 2;
                return _services_authService__WEBPACK_IMPORTED_MODULE_20__["default"].getCurrentUser();

              case 2:
                user = _context5.sent;
                jwt = _services_authService__WEBPACK_IMPORTED_MODULE_20__["default"].getJwt();
                this.setState({
                  user: user,
                  jwt: jwt
                });
                this.refreshData();

              case 6:
              case "end":
                return _context5.stop();
            }
          }
        }, _callee5, this);
      }));

      function componentDidMount() {
        return _componentDidMount.apply(this, arguments);
      }

      return componentDidMount;
    }()
  }, {
    key: "render",
    value: function render() {
      var _this2 = this;

      return __jsx(_components_layout__WEBPACK_IMPORTED_MODULE_17__["default"], {
        __source: {
          fileName: _jsxFileName,
          lineNumber: 229
        },
        __self: this
      }, __jsx("div", {
        className: "w-50 mx-auto text-center pt-5",
        __source: {
          fileName: _jsxFileName,
          lineNumber: 230
        },
        __self: this
      }, __jsx("img", {
        src: _src_crypto_text_png__WEBPACK_IMPORTED_MODULE_23___default.a,
        __source: {
          fileName: _jsxFileName,
          lineNumber: 231
        },
        __self: this
      }), __jsx("h1", {
        style: {
          fontFamily: "Courier New"
        },
        __source: {
          fileName: _jsxFileName,
          lineNumber: 232
        },
        __self: this
      }, __jsx("strong", {
        __source: {
          fileName: _jsxFileName,
          lineNumber: 232
        },
        __self: this
      }, "Saved Data")), __jsx("hr", {
        __source: {
          fileName: _jsxFileName,
          lineNumber: 233
        },
        __self: this
      }), this.state.loaded && __jsx("div", {
        className: "col-md-12 mt-3",
        __source: {
          fileName: _jsxFileName,
          lineNumber: 234
        },
        __self: this
      }, __jsx("p", {
        __source: {
          fileName: _jsxFileName,
          lineNumber: 235
        },
        __self: this
      }, __jsx("strong", {
        __source: {
          fileName: _jsxFileName,
          lineNumber: 235
        },
        __self: this
      }, "Refresh")), __jsx("button", {
        className: "btn btn-success",
        onClick: this.refreshData,
        __source: {
          fileName: _jsxFileName,
          lineNumber: 236
        },
        __self: this
      }, __jsx("i", {
        className: "fas fa-sync",
        __source: {
          fileName: _jsxFileName,
          lineNumber: 236
        },
        __self: this
      })))), this.state.loaded ? __jsx("div", {
        className: "text-center",
        __source: {
          fileName: _jsxFileName,
          lineNumber: 240
        },
        __self: this
      }, this.state.graphs.length > 0 ? this.state.graphs.map(function (item, index) {
        return __jsx("div", {
          key: item.graphId,
          className: "pt-3",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 243
          },
          __self: this
        }, __jsx("hr", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 244
          },
          __self: this
        }), __jsx("img", {
          src: item.image,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 245
          },
          __self: this
        }), __jsx("div", {
          className: "row mt-4",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 246
          },
          __self: this
        }, __jsx("div", {
          className: "col-md-9 text-center",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 247
          },
          __self: this
        }, !_this2.state.loading && !_this2.state.loadArr[index] ? __jsx(highcharts_react_official__WEBPACK_IMPORTED_MODULE_19___default.a, {
          key: item.id,
          highcharts: highcharts__WEBPACK_IMPORTED_MODULE_18___default.a,
          options: item.chartOptions,
          allowChartUpdate: true,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 248
          },
          __self: this
        }) : __jsx("div", {
          className: "mx-auto text-center",
          style: {
            paddingTop: "20%"
          },
          __source: {
            fileName: _jsxFileName,
            lineNumber: 248
          },
          __self: this
        }, __jsx("img", {
          src: _src_91_gif__WEBPACK_IMPORTED_MODULE_22___default.a,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 248
          },
          __self: this
        }))), __jsx("div", {
          className: "col-md-3",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 250
          },
          __self: this
        }, __jsx("div", {
          className: "form-group",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 251
          },
          __self: this
        }, __jsx("div", {
          className: "pt-2",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 252
          },
          __self: this
        }, __jsx("span", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 253
          },
          __self: this
        }, __jsx("strong", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 253
          },
          __self: this
        }, "Cryptocurrency"))), __jsx("div", {
          className: "pt-2",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 255
          },
          __self: this
        }, __jsx("input", {
          type: "text",
          value: item.cryptocurrency.name,
          className: "form-control text-center shadow",
          disabled: true,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 256
          },
          __self: this
        })), __jsx("div", {
          className: "pt-2",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 258
          },
          __self: this
        }, __jsx("span", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 259
          },
          __self: this
        }, __jsx("strong", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 259
          },
          __self: this
        }, "To currency"))), __jsx("div", {
          className: "pt-2",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 261
          },
          __self: this
        }, __jsx("input", {
          type: "text",
          value: item.currency.name,
          className: "form-control text-center shadow",
          disabled: true,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 262
          },
          __self: this
        })), __jsx("div", {
          className: "row",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 264
          },
          __self: this
        }, __jsx("div", {
          className: "col-md-12 mt-3",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 265
          },
          __self: this
        }, __jsx("p", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 266
          },
          __self: this
        }, __jsx("strong", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 266
          },
          __self: this
        }, "Compare with current data")), __jsx("input", {
          type: "checkbox",
          onChange: function onChange(e) {
            return _this2.compare(e, index);
          },
          className: "form-control",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 267
          },
          __self: this
        })), __jsx("div", {
          className: "col-md-6 mt-3",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 269
          },
          __self: this
        }, __jsx("p", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 270
          },
          __self: this
        }, __jsx("strong", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 270
          },
          __self: this
        }, "Delete")), __jsx("button", {
          className: "btn btn-danger",
          onClick: function onClick() {
            return _this2.editGraph(index, "remove");
          },
          __source: {
            fileName: _jsxFileName,
            lineNumber: 271
          },
          __self: this
        }, __jsx("i", {
          className: "fas fa-minus",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 271
          },
          __self: this
        }))), __jsx("div", {
          className: "col-md-6 mt-3",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 273
          },
          __self: this
        }, __jsx("p", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 274
          },
          __self: this
        }, __jsx("strong", {
          __source: {
            fileName: _jsxFileName,
            lineNumber: 274
          },
          __self: this
        }, "Save")), __jsx("button", {
          className: "btn btn-primary",
          onClick: function onClick() {
            return _this2.editGraph(index, "save");
          },
          disabled: !_this2.state.compare,
          __source: {
            fileName: _jsxFileName,
            lineNumber: 275
          },
          __self: this
        }, __jsx("i", {
          className: "fas fa-save",
          __source: {
            fileName: _jsxFileName,
            lineNumber: 275
          },
          __self: this
        }))))))));
      }) : "") : __jsx("div", {
        className: "w-50 mx-auto text-center",
        style: {
          paddingTop: "30vh"
        },
        __source: {
          fileName: _jsxFileName,
          lineNumber: 285
        },
        __self: this
      }, __jsx("img", {
        src: _src_91_gif__WEBPACK_IMPORTED_MODULE_22___default.a,
        __source: {
          fileName: _jsxFileName,
          lineNumber: 285
        },
        __self: this
      })));
    }
  }]);

  return SavedData;
}(react__WEBPACK_IMPORTED_MODULE_16__["Component"]);



/***/ })

})
//# sourceMappingURL=saveddata.js.38fac1bf46caf4e5945b.hot-update.js.map